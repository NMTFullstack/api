...
exports.getDataCategoryTv365 = async (req, res, next) => {
    try {
        ...
        const category = await CategoryTv365.find(condition).sort({
            cat_name: 1,
        });
    ...
    try {
        ...
        if (list_id) {
            ...
            if (type == 1) {
                ...
                return fnc.success(res, "Danh sách online", {
                    data: list,
                    total: list.length,
                });
            } else if (type == 0) {
    ...
    try {
        ...
        if (list_id) {
            ...
            let listUser = await Users.find(
                { _id: { $in: listUserIdFinal }, idTimViec365: { $ne: 0 } },
                {
                    password: 0,
                    configChat: 0,
            ...
            for (let i = 0; i < listNtd.length; i++) {
                ...
                let flag = true;
                let new_title = "";
                let news = listNew.find(
                    (e) => e.new_user_id == obj.idTimViec365
                );
                if (news) {
                ...
                if (flag) {
                    dataNtdFinal.push({
                        chat365_id: obj._id,
                        usc_logo: functions.getUrlLogoCompany(
                            obj.createdAt,
                            obj.avatarUser
                        ),
            ...
            for (let i = 0; i < listUv.length; i++) {
                ...
                if (
                    obj.inForPerson &&
                    obj.inForPerson.account &&
                    obj.inForPerson.account.experience
                ) {
                ...
                if (req.body.keyword) {
                    ...
                    if (
                        !cv_title.includes(keyword) &&
                        !obj.userName.includes(keyword)
                    ) {
                        flag = false;
                    }
                }
                if (req.body.city_id) {
                    if (
                        !cv_city_id.find((e) => e == Number(req.body.city_id))
                    ) {
                        flag = false;
                    }
                }
                ...
                if (flag) {
                    dataUvFinal.push({
                        ...
                        use_logo: functions.getUrlLogoCompany(
                            obj.createdAt,
                            obj.avatarUser
                        ),
