        ...
        if (password && phoneTK) {
            ...
            if (checkPhone) {
                ...
                if (!finduser) {
                    ...
                    return functions.success(res, 'Đăng ký thành công', { data, id: user.idQLC })
                }
            ...
            if (checkPhone) {
                ...
                if (checkTypeUser.type == 0) {
                    ...
                    if (checkTypeUser != null) {
                        ...
                        const refreshToken = await functions.createToken({ userId: checkTypeUser._id },
                            '1y'
                        )
                        let data = {
                            access_token: token,
...
exports.CheckUpdatePasswordByInput = async(req, res, next) => {
    try {
        ...
        if (input) {
            ...
            if (user) {
                return functions.success(res, ' tài khoản tồn tại ', {user: {id: user._id}});
            }
...
exports.isExists = async(req, res) => {
    try {
        ...
        let user = await Users.findOne({ idQLC: com_id, type: 1 })
        let departments = await Deparment.find({ com_id: com_id })
