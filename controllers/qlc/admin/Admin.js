...
exports.listComSummary = async(req, res) => {
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            element.count_empUnTimeKeeping = element.count_emp - element.count_empTimeKeeping;
        }
...
exports.getBlog = async (req, res, next) => {
    try {
        ...
        if (title) condition1["title"] = { $regex: title, $options: 'i'}; //tìm kiếm theo tiêu đề
        if (authorName) condition2["userName"] = { $regex: authorName, $options: 'i'}; //tìmm kiếm theo tên tác giả
        ...
        if (data.length == 0) return functions.success(res, "Không có bài viết nào", { data: [], count: 0 });
        ...
        return functions.success(res, "Lấy thành công", { data, count: count[0].total });
    } catch (err) {
        console.log(err);
...
exports.createBlog = async (req, res, next) => {
    try {
        ...
        if (
            (title && content && description && id_author && keyWord)
        ) {
            const findAuthor = await user.findOne({ idQLC: id_author, type: 2});
            if (findAuthor) {
            ...
            let _idBlog = 0;
            const maxID = await blog.findOne({}, { _id: 1 })
                .sort({ _id: -1 })
                .limit(1)
                .lean();
            if (maxID) {
