...
exports.createLiquidationAssetProposal = async (req, res, next) => {
  try {
    ...
    if (sl_tl && lydo_thanhly && tentstl) {
      let check_gt_ts = await TaiSan.findOne(
        { ts_id: tentstl, id_cty: comId },
        { ts_gia_tri: 1 }
      )
      if (check_gt_ts) {
...
exports.getDataLiquidationAssetProposal = async (req, res, next) => {
  try {
    ...
    if (type_quyen === 2) {
      conditions = { $or: [{ id_ng_tao: emId }, { id_ngdexuat: emId }] }
    }
    ...
    for (let i = 0; i < taiSanDeXuatThanhLy.length; i++) {
      ...
      taiSanDeXuatThanhLy[i].ngaytao = new Date(
        taiSanDeXuatThanhLy[i].ngaytao * 1000
      )
      ...
      if (tl_type_quyen === 1) {
        let user = await Users.findOne(
          { idQLC: taiSanDeXuatThanhLy[i].ngdexuat, type: 1 },
          { userName: 1, inForPerson: 1, address: 1 }
        )
        taiSanDeXuatThanhLy[i].ngdexuat = user.userName
        taiSanDeXuatThanhLy[i].phongban = user.userName
      } else {
        let user = await Users.findOne(
          { _id: taiSanDeXuatThanhLy[i].ngdexuat },
          { userName: 1, inForPerson: 1, address: 1 }
        )
        if (user) {
          taiSanDeXuatThanhLy[i].ngdexuat = user.userName
          if (
            ...
            let dep = await OrganizeDetail.findOne(
              { id: user.inForPerson.employee.organizeDetailId },
              { organizeDetailName: 1 }
            )
            if (dep) {
              taiSanDeXuatThanhLy[i].phongban = dep.organizeDetailName
            }
          }
        }
      }
    ...
    return functions.success(res, 'get data success', {
      countTaiSanDeXuatThanhLy,
      countTaiSanDaThanhLy,
      data: taiSanDeXuatThanhLy,
    })
  } catch (error) {
    console.error(error)
...
exports.excelDXThanhLy = async (req, res, next) => {
  try {
    ...
    for (let i = 0; i < taiSanDeXuatThanhLy.length; i++) {
      ...
      // trả về định dạng ngày
      if (taiSanDeXuatThanhLy[i].ngaytao != 0) {
        const newDate = new Date(taiSanDeXuatThanhLy[i].ngaytao * 1000)
        taiSanDeXuatThanhLy[i].ngaytao = `${newDate.getDate()}-${
          newDate.getMonth() + 1
        }-${newDate.getFullYear()}`
      }
      if (taiSanDeXuatThanhLy[i].trangthai === 0)
        taiSanDeXuatThanhLy[i].trangthai = 'Chờ duyệt'
      else taiSanDeXuatThanhLy[i].trangthai = 'Từ chối'
    }

    const DeXuatThanhLy_xlsx = [
      [
        'STT',
        'Số biên bản',
        'Ngày tạo',
    ...
    for (let i = 0; i < taiSanDeXuatThanhLy.length; i++) {
      const row = [
        i + 1,
        taiSanDeXuatThanhLy[i].sobienban,
        taiSanDeXuatThanhLy[i].ngaytao,
        taiSanDeXuatThanhLy[i].trangthai,
        taiSanDeXuatThanhLy[i].mataisan,
        taiSanDeXuatThanhLy[i].tentaisan,
        taiSanDeXuatThanhLy[i].soluong,
        taiSanDeXuatThanhLy[i].ngdexuat,
        taiSanDeXuatThanhLy[i].phongban,
        taiSanDeXuatThanhLy[i].lydo,
      ]
      DeXuatThanhLy_xlsx.push(row)
    }
    quanlytaisanService.excel(DeXuatThanhLy_xlsx, 'DanhSachDeXuatThanhLy', res)
  } catch (error) {
    console.log(error)
    return functions.setError(res, error.message)
  }
...
exports.approveLiquidationAssetProposal = async (req, res, next) => {
  try {
    ...
    if (
      bb_tl &&
      [0, 1].includes(chon_gt_tl) &&
      functions.checkTime(day_thanhly) &&
      soluong_thanhly &&
      lydo_thanhly
    ) {
      ...
      if (checkThanhLy) {
        ...
        if (quyen_nhan === 1) {
          let doi_tuong_ds_ts = await TaiSan.findOne({
            id_cty: comId,
            ts_id: checkThanhLy.thanhly_taisan,
          })
          ...
          if (doi_tuong_ds_ts) {
            ...
            if (sl_ts_dang_sd > soluong_thanhly) {
              ...
              let quatrinh_id = await functions.getMaxIdByField(
                QuaTrinhSuDung,
                'quatrinh_id'
              )
          ...
          let doi_tuong_ds_ts = await TaiSanDangSuDung.findOne({
            com_id_sd: comId,
            id_ts_sd: checkThanhLy.thanhly_taisan,
            id_nv_sd: id_ng_nhan,
          })
          ...
          return functions.setError(
            res,
            'Không tìm thấy tài sản đang sử dụng',
            404
          )
  ...
  try {
    ...
    if (checkThanhLy) {
      ...
      return functions.success(
        res,
        'Từ chối đề xuất thanh lý tài sản thành công'
      )
    }
    return functions.setError(
      res,
      'Không tìm thấy đề xuất thanh lý tài sản',
      404
    )
  } catch (error) {
    return functions.setError(res, error)
  ...
  try {
    ...
    if (type === 1) {
      ...
      if (Array.isArray(id) === true) {
        ...
        return functions.success(
          res,
          'Xoá 1 đề xuất thanh lý tài sản thành công'
        )
      }
    } else if (type === 2) {
      if (Array.isArray(id) === true) {
        ...
        return functions.success(
          res,
          'Khôi phục đề xuất thanh lý tài sản thành công'
        )
      } else {
        await ThanhLy.findOneAndUpdate(
          { tl_id: Number(id) },
        ...
        return functions.success(
          res,
          'Khôi phục 1 đề xuất thanh lý tài sản thành công'
        )
      }
    } else if (type === 3) {
      if (Array.isArray(id) === true) {
        for (let i = 0; i < id.length; i++) {
          await ThanhLy.findOneAndDelete({ tl_id: id[i] })
        }
        return functions.success(
          res,
          'Xoá vĩnh viễn đề xuất thanh lý tài sản thành công'
        )
      } else {
        await ThanhLy.findOneAndDelete({ tl_id: Number(id) })
        return functions.success(
          res,
          'Xoá vĩnh viễn 1 đề xuất thanh lý tài sản thành công'
        )
      }
    }
      ...
      if (chiTiet.length !== 0) {
        ...
        if (user) {
          ...
          if (tl_type_quyen === 2) {
            ...
            chiTiet.doi_tuong_sd = user.userName
            if (dep) {
              chiTiet.phongban = dep.organizeDetailName
              chiTiet.vitri = dep.organizeDetailName
            }
          }
          if (tl_type_quyen === 3) {
            let dep = await OrganizeDetail.findOne(
            ...
            if (dep) {
              chiTiet.nguoitao = dep.organizeDetailName
              chiTiet.ngdexuat = dep.organizeDetailName
              chiTiet.phongban = dep.organizeDetailName
              chiTiet.vitri = dep.organizeDetailName
              chiTiet.doi_tuong_sd = dep.organizeDetailName
            }
          }
...
exports.getDataDidLiquidationAssetProposal = async (req, res, next) => {
  try {
    ...
    if (keywords) conditions.tl_id = Number(keywords)
    ...
    for (let i = 0; i < taiSanDeXuatThanhLy.length; i++) {
      // trả về định dạng ngày
      taiSanDeXuatThanhLy[i].ngayduyet = new Date(
        taiSanDeXuatThanhLy[i].ngayduyet * 1000
      )
      taiSanDeXuatThanhLy[i].ngaythanhly = new Date(
        taiSanDeXuatThanhLy[i].ngaythanhly * 1000
      )
      // lấy quyền người dùng đã đề xuất
      let tl_type_quyen = taiSanDeXuatThanhLy[i].tl_type_quyen
      // lấy thông tin user
      ...
      if (tl_type_quyen === 2) {
        ...
        if (user) {
          ...
          if (dep) taiSanDeXuatThanhLy[i].phongban = dep.organizeDetailName
          else taiSanDeXuatThanhLy[i].phongban = 'chưa cập nhật'
        }
      }
      if (tl_type_quyen === 3) {
        let dep = await OrganizeDetail.findOne(
          { id: taiSanDeXuatThanhLy[i].ngdexuat },
          { organizeDetailName: 1 }
        )
        if (dep) {
          taiSanDeXuatThanhLy[i].ngdexuat = dep.organizeDetailName
          taiSanDeXuatThanhLy[i].phongban = dep.organizeDetailName
        }
      }
    }
...
exports.excelDaThanhLy = async (req, res, next) => {
  try {
    const com_id = Number(req.params._id)
    let conditions = {}
    ...
    for (let i = 0; i < taiSanDeXuatThanhLy.length; i++) {
      ...
      if (taiSanDeXuatThanhLy[i].ngayduyet != 0) {
        const newDate = new Date(taiSanDeXuatThanhLy[i].ngayduyet * 1000)
        taiSanDeXuatThanhLy[i].ngayduyet = `${newDate.getDate()}-${
          newDate.getMonth() + 1
        }-${newDate.getFullYear()}`
      }
      if (taiSanDeXuatThanhLy[i].ngaythanhly != 0) {
        const newDate = new Date(taiSanDeXuatThanhLy[i].ngaythanhly * 1000)
        taiSanDeXuatThanhLy[i].ngaythanhly = `${newDate.getDate()}-${
          newDate.getMonth() + 1
        }-${newDate.getFullYear()}`
      }
      if (taiSanDeXuatThanhLy[i].trangthai === 0)
    ...
    for (let i = 0; i < taiSanDeXuatThanhLy.length; i++) {
      const row = [
        ...
        taiSanDeXuatThanhLy[i].phongban,
        taiSanDeXuatThanhLy[i].lydo,
        taiSanDeXuatThanhLy[i].ngayduyet,
        taiSanDeXuatThanhLy[i].ngaythanhly,
        taiSanDeXuatThanhLy[i].tienthanhly,
      ]
      DeXuatThanhLy_xlsx.push(row)
    }
    quanlytaisanService.excel(DeXuatThanhLy_xlsx, 'DanhSachDeXuatThanhLy', res)
