    ...
    try {
        ...
        if (username && password && emailContact && phoneTK) {
            ...
            if (CheckPhoneNumber && CheckEmail) {
                ...
                let user = await functions.getDatafindOne(Users, { phoneTK, type: 1 });
                if (user == null) {
                    ...
                    let companyUnset = await functions.getDatafindOne(CompanyUnset, { phoneTK })
                    if (companyUnset != null) {
                        await functions.getDataDeleteOne(CompanyUnset, { phoneTK })
                    }
    ...
    try {
        if (req.body.email && req.body.password) {
            ...
            if (checkPhoneNumber) {
                let findUser;
                if (email.includes('@')) {
                    findUser = await functions.getDatafindOne(Users, { email, type: 1 });
                } else {
                    findUser = await functions.getDatafindOne(Users, { phoneTK: email, type: 1 });
                }
                if (findUser) {
    ...
    try {
        ...
        if (phone && userCompany && city && district && address) {
            ...
            if (checkPhone) {
                ...
                if (usc_founded_time) {
                    dataUpdate["inForCompany.timviec365.usc_founded_time"] = usc_founded_time;
                }
                ...
                if (usc_branches_edit && usc_branches_edit.length) {
                    ...
                    usc_branches_edit.forEach(payload => {
                        if (payload._id) {
                            promises.push(
                                Users.updateOne({ _id: companyID, "inForCompany.timviec365.usc_branches._id": payload._id }, {
                                    $set: {
    ...
    try {
        ...
        if (JSON.stringify(req.files) !== '{}') {
            ...
            for (let index = 0; index < storage.length; index++) {
                ...
                if (service.checkItemStorage(file.type)) {
                    if (service.isImage(file.type)) {
                        uploadStorage = service.uploadStorage(com_id, file, 'image', user.createdAt);
                        await service.addStorage(com_id, 'image', uploadStorage.file_name);

                    } else {
                        uploadStorage = service.uploadStorage(com_id, file, 'video', user.createdAt);
...
exports.uploadAvatar = async (req, res, next) => {
    try {
        const user = req.user.data,
            file = req.files;
        if (file) {
            const avatarUser = service.uploadLogo(file.avatarUser, user.createdAt);
            if (avatarUser) {
...
exports.listUVApplyJob = async (req, res, next) => {
    try {
        ...
        if (list.length > 0) {
            list.forEach(user => {
                user.use_logo = functions.cdnImageAvatar(Number(user.use_create_time) * 1000) + user.use_logo;
            })
...
exports.listSaveUV = async (req, res, next) => {
    try {
        ...
        const totalCount = await functions.findCount(SaveCandidate, { usc_id: Number(idCompany) });
        return functions.success(res, "Lấy danh sách uv thành công", { total: totalCount, items: list });
    } catch (error) {
        console.log(error)
        return functions.setError(res, error)
    }
...
exports.seenUVWithPoint = async (req, res, next) => {
    try {
        ...
        if (idUser) {
            const candidate = await Users.findOne({ idTimViec365: idUser }).select("userName idTimViec365");
            if (!candidate) return functions.setError(res, 'Ứng viên không tồn tại', 200);
            ...
            if (!checkUsePoint) {
                ...
                if (companyPoint) {
                    ...
                    if (type_payment === 0) {
                        ...
                        const _price = 30000;
                        if (money_usc >= _price) {
                            await CreditController.useCreditsHandler(idCompany, _price, 1, candidate.userName, candidate.idTimViec365);
                            const pointUsed = new PointUsed({
                                usc_id: idCompany,
                                use_id: idUser,
                                money: _price,
        ...
        if (list_id) {
            ...
            if (list_img.length > 0) {
                ...
                for (let j = 0; j < list_img.length; j++) {
                    ...
                    const ObjectId = mongoose.Types.ObjectId;
                    await CompanyStorage.deleteOne({ _id: new ObjectId(element._id) });
                }
...
exports.listNews = async (req, res, next) => {
    try {
        ...
        const skip = (page - 1) * pageSize;
        const limit = pageSize;
        const condition = { new_user_id: idCompany, $or: [{ new_md5: null }, { new_md5: '' }] };
        ...
        const dv_bl = await TblBaoLuu.find({ id_ntd_bl: idCompany }).sort({ _id: -1 });
        return functions.success(res, "Lấy danh sách tin đăng thành công", { total, items: listPost, dv_bl });
...
exports.updateUVPoint = async (req, res, next) => {
    try {
        ...
        if (userID && idCompany) {
            ...
            return functions.setError(res, 'người dùng hoặc công ty không tồn tại', 404)
        }
        return functions.setError(res, 'không đủ dữ liệu', 404)
...
exports.deleteUVUsePoint = async (req, res, next) => {
    try {
        ...
        let idUser = req.body.user_id;
        if (idUser) {
            await functions.getDataDeleteOne(PointUsed, { usc_id: idCompany, use_id: idUser })
            return functions.success(res, 'xóa thành công',)
        }
        return functions.setError(res, 'không đủ dữ liệu', 404)
    } catch (error) {
...
exports.listSaveUVAll = async (req, res, next) => {
    try {
        ...
        const totalCount = await functions.findCount(SaveCandidate, { usc_id: idCompany });
        return functions.success(res, "Lấy danh sách uv thành công", { total: totalCount, items: list });
    } catch (error) {
        console.log(error)
        return functions.setError(res, error)
