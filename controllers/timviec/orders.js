    ...
    try {
        ...
        if (checkUser) {
            ...
            if (adminUser) {
                ...
                let list_service = await PriceList.find({
                    bg_id: { $in: arr_id_service },
                });
                if (
                    (list_service && list_service.length > 0) ||
                    arr_id_service_crm.length > 0
                ) {
                    ...
                    let detail_id = 0;
                    let latestOrderDetails = await OrderDetails.findOne()
                        .sort({ id: -1 })
                        .lean();
                    ...
                    for (let i = 0; i < arr_product.length; i++) {
                        ...
                        if (product.type_service != 7) {
                            ...
                            serviceDetail = service.renderServiceDetailCRM(
                                product.id_service,
                    ...
                    await service.sendMessageToSupporter(
                        adminUser.emp_id,
                        code_order,
                        name,
                        phone,
                    ...
                    if (detail) {
                        ...
                        let namepdf = `../storage/base365/timviec365/pdforder/order_${Number(
                            id_order
                        )}.pdf`;
                        let nameimg = `../storage/base365/timviec365/imageorder/order_${Number(
                            id_order
                        )}.png`;
...
exports.orderProduct = async (req, res, next) => {
    try {
        if (
            ...
            let result = await handleOrderProduct(
                id_user,
                type_user,
                name,
                phone,
            ...
            if (result) {
                return functions.success(res, "Đặt hàng thành công", {
                    data: { result: true },
                });
            } else {
                return functions.setError(
                    res,
                    "Thông tin đặt hàng không đúng",
                    400
                );
            }
        } else {
...
exports.getPromotionalPoints = async (req, res) => {
    try {
        if (!req.user && !req.user.data)
            return functions.setError(res, "Không có dữ liệu người dùng", 400);
        ...
        return functions.success(res, "Lấy điểm khuyến mãi thành công", {
            data: { point: availablePoints > 0 ? availablePoints : 0 },
        });
    } catch (error) {
        console.log(error);
        return functions.setError(res, "Đã có lỗi xảy ra", 400);
...
exports.getDiscountData = async (req, res) => {
    try {
        ...
        if (!userId || !userType)
            return functions.setError(res, "Dữ liệu truyền lên không đủ", 400);
        userId = Number(userId);
        userType = Number(userType);
        let [pointPromo, pointOrder, pointUsed, managerHistory] =
            await Promise.all([
...
exports.getPackageInfo = async (req, res) => {
    try {
        let { bg_type, bg_id, userId } = req.body;
        if (!bg_type && !bg_id)
            return functions.setError(
                res,
                "Thông tin truyền lên không đầy đủ",
        ...
        let priceList = [];
        if (bg_id) {
            priceList = await PriceList.find({ bg_id }).select(
                "-bg_quyenloi -bg_uudai1 -bg_uudai2 -bg_uudai3 -bg_cm1 -bg_cm2 -bg_cm3 -bg_cm_logo"
            );
        } else {
            priceList = await PriceList.find({ bg_type }).select(
                "-bg_quyenloi -bg_uudai1 -bg_uudai2 -bg_uudai3 -bg_cm1 -bg_cm2 -bg_cm3 -bg_cm_logo"
            );
        }
        ...
        if (["1", "3", "4", "5", "6"].includes(String(bg_type)) && userId) {
            newsList = await NewTV365.find({ new_user_id: userId })
                .sort({ new_update_time: -1 })
                .select("new_title new_id");
        }
        return functions.success(res, "Thành công", {
            data: {
                bg_type: bg_type,
...
exports.getOrderHistory = async (req, res) => {
    try {
        let { status, limit, page, userId, userType } = req.body;
        ...
        let aggrData = await Order.aggregate([
            ...
            },
            {
                $facet: {
                    total: [{ $count: "count" }],
                    pending: [{ $match: { status: 0 } }, { $count: "count" }],
                    active: [{ $match: { status: 1 } }, { $count: "count" }],
                    completed: [{ $match: { status: 2 } }, { $count: "count" }],
                    expired: [{ $match: { status: 3 } }, { $count: "count" }],
                    cancelled: [{ $match: { status: 4 } }, { $count: "count" }],
                    listTotal: [{ $match: match }, { $count: "count" }],
                    list: [
                        { $match: match },
                        { $sort: { create_time: -1 } },
                        { $skip: skip },
...
exports.getOrderDetails = async (req, res) => {
    try {
        ...
        if (!orderData.length)
            return functions.setError(res, "Không có đơn hàng này", 404);
...
exports.getVipData = async (req, res) => {
    try {
        ...
        console.log(pointPromo, pointOrder, pointUsed, managerHistory);
        let managerPoints = 0;
        let pointToChange = 0;
        let usedPoints = 0;
        let exchangedMoney = 0;
        ...
        if (user.inForCompany && user.inForCompany.timviec365)
            vip = user.inForCompany.timviec365.usc_vip;

        return functions.success(res, "Thành công", {
            data: { point: available_points, vip: vip ? vip : 0 },
        });
    } catch (error) {
        console.log(error);
        return functions.setError(res, "Đã có lỗi xảy ra", 400);
