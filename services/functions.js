...
exports.checkVideo = async (filePath) => {
  // kiểm tra loại file
  if (
    !allowedTypes.includes(path.extname(filePath.originalname).toLowerCase())
  ) {
    return false
  }
...
exports.uploadVideoAndIMGRegister = multer(
  { storage: storageFile('public/company') },
  (fileFilter = function (req, file, cb) {
    const allowedTypes = [
      'image/jpeg',
      'image/png',
...
exports.saveImg = (img) => {
  ...
  fs.writeFileSync(pathname + '/' + time + '.png', image)

  return `https://cdn.timviec365.vn/hh365/blogadmin/${time}.png`
}
